...
export class NetworkClient {
  ...
  connect() {
    // When running as an Electron desktop app, window.__gameServer__ overrides the host.
    const server = (typeof window !== 'undefined' && window.__gameServer__) || location.host;
    const proto  = (server !== location.host || location.protocol === 'https:') ? 'wss:' : 'ws:';
    this._proto = proto;
    try {
      this.ws = new WebSocket(`${proto}//${server}`);
    } catch {
      return;
    }
  ...
  _reconnect() {
    ...
    try {
      const server = (typeof window !== 'undefined' && window.__gameServer__) || location.host;
      this.ws = new WebSocket(`${this._proto}//${server}`);
      this.ws.onopen = () => {
        this.connected = true;
