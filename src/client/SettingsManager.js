...
export class SettingsManager {
  ...
  /** Show name-entry prompt. resolves when user submits. */
  promptName() {
    return new Promise(resolve => {
      const el = document.getElementById('name-prompt');
      if (!el) { resolve(this._data.name); return; }
      el.style.display = 'flex';

      const input = document.getElementById('name-input');
      if (input) input.value = this._data.name !== 'Player' ? this._data.name : '';

      const submit = () => {
        const val = input?.value.trim().slice(0, 20) || 'Player';
        this._data.name = val;
        this._save();
      ...
      document.getElementById('name-submit')?.addEventListener('click', submit, { once: true });
