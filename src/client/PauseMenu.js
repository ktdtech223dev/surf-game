...
export class PauseMenu {
  ...
  _renderSidebar() {
    ...
    sb.innerHTML = `
      ...
      ${btn('pm-radio',    '📻', 'Radio')}
      ${window.electron ? btn('pm-display', '🖥', 'Display') : ''}
    ...
    document.getElementById('pm-display')?.addEventListener('click', () => {
      this._panel = 'display'; this._renderPanel(); this._highlightNav('pm-display');
    });
  ...
  _renderDisplayPanel(panel) {
    const modes = [
      { id: 'windowed',   label: 'Windowed',            desc: 'Fixed window, resizable' },
      { id: 'borderless', label: 'Borderless Fullscreen', desc: 'Frameless, covers full screen' },
      { id: 'fullscreen', label: 'Fullscreen',           desc: 'Exclusive fullscreen' },
    ];

    const savedMode = localStorage.getItem('cuunsurf_winmode') || 'windowed';

    const row = (m) => `
      <div id="dm-${m.id}" data-mode="${m.id}" style="
        display:flex;align-items:center;gap:16px;padding:14px 16px;margin-bottom:6px;
        border-radius:8px;cursor:pointer;transition:all 0.12s;
        background:${savedMode === m.id ? 'rgba(0,207,255,0.1)' : 'rgba(255,255,255,0.02)'};
        border:1px solid ${savedMode === m.id ? 'rgba(0,207,255,0.3)' : 'rgba(255,255,255,0.05)'};
      ">
        <div style="width:14px;height:14px;border-radius:50%;flex-shrink:0;
          background:${savedMode === m.id ? '#00cfff' : '#222'};
          ${savedMode === m.id ? 'box-shadow:0 0 8px #00cfff' : ''}
        "></div>
        <div>
          <div style="font-size:13px;color:${savedMode === m.id ? '#00cfff' : '#bbb'};font-weight:${savedMode === m.id ? 'bold' : 'normal'}">${m.label}</div>
          <div style="font-size:9px;color:#334;letter-spacing:1px;margin-top:2px">${m.desc}</div>
        </div>
      </div>
    `;
    ...
    panel.innerHTML = `
      <div>
        <div style="font-size:10px;letter-spacing:4px;color:#2a2a4a;margin-bottom:20px">DISPLAY MODE</div>
        ${modes.map(row).join('')}
        <div style="margin-top:20px;font-size:9px;color:#1a1a2e;letter-spacing:1px;border-top:1px solid #111;padding-top:12px">
    ...
    panel.querySelectorAll('[data-mode]').forEach(el => {
      el.addEventListener('mouseenter', () => {
        if (el.dataset.mode !== (localStorage.getItem('cuunsurf_winmode') || 'windowed'))
      ...
      el.addEventListener('click', () => {
        ...
        localStorage.setItem('cuunsurf_winmode', mode);
        window.electron?.setWindowMode(mode);
        this._renderDisplayPanel(panel); // re-render to update selection
      });
    });
  }

  _highlightNav(activeId) {
    ['pm-mapsel','pm-settings','pm-stats','pm-crosshair','pm-radio','pm-display'].forEach(id => {
  ...
  _renderPanel() {
    ...
    switch (this._panel) {
      ...
      case 'radio':
        ...
        break;
      case 'display':
        this._renderDisplayPanel(panel);
        break;
      default:
